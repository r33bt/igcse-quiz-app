...
export default function PracticeQuizPage({ params, searchParams }: PageProps) {
  ...
  const handleQuizComplete = async (results: any) => {
    try {
      console.log('📊 Practice quiz completed, updating IGCSE progress...', results)
      ...
      console.log('📈 Processed practice results:', quizResults)
      ...
      setTimeout(() => {
        window.location.href = '/test-topics?completed=practice&subtopic=' + subtopicId + '&focus=' + (focus || 'mixed')
      }, 2000)
      
    } catch (error) {
      console.error('❌ Failed to update progress:', error)
      alert('Practice completed but failed to save progress. Please try again.')
    }
  }
